...
- Each selected slice uses a fixed four-way-overlap granular synthesis hop. The analysis hop is the synthesis hop divided by a bounded amount-derived stretch ratio, while samples inside each grain advance at `+1` or `-1`. This creates pitch-preserving time expansion rather than variable-speed pitch shifting. Fractional ring reads are linear; normalization moves from coherent window-sum toward overlap-energy compensation as stretch decorrelates grains. A fixed 24-probe comparison applies at most `0.80x`–`1.25x` per-slice compensation so random region choice does not become a large level jump, and a 6 ms equal-power slice-edge taper crossfades with dry audio.
- MELT Amount controls a two-to-four-slice progression, 24–42 ms target grain size, wet level, a randomized `1.08x`–`4x` stretch range, and an internal per-slice reversal probability rising from about 8% to 60%. Both channels share all timing, ratio, origin, and direction choices to preserve stereo geometry.
...
- SMEAR allocates one second of stereo raw/medium/high filtered history, sixteen fixed grain records, and fixed sine/Hann lookup tables in `prepareToPlay`. Amount raises the target active population from about two to sixteen while broadening grain lengths from roughly 60–100 ms toward 8–30 ms. Grains start only when a safe read-behind distance exists, use tapered windows, musically restricted pitch intervals, bounded scatter, lifetime-relative pitch/pan orbit, stereo panning/crossfeed, and high-passed residual emphasis.
