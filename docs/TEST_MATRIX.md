...
The compact-editor code head `4a2bae8` passed [run #55](https://github.com/idkanonto/samplerthingidk/actions/runs/34175784343); CTest passed 1/1 in 0.16 seconds and artifact `10037311950` contained the workflow-verified 7,466,496-byte Windows module. Creative-quality code head `d301d34` passed [run #60](https://github.com/idkanonto/samplerthingidk/actions/runs/34195038905); CTest passed 1/1 in 0.83 seconds, both artifacts verified against their GitHub SHA-256 digests, and the VST3 contained a 7,460,352-byte Windows module.
...
| SCRAMBLE | Exact bypass, deterministic gesture selection, continuous activity, monotonic chaos budget, integrated pitch/hold/reverse behavior, discontinuity, finite bounds | Run #60 passed low activation, identical fixed-seed output, successive-grid activation without history loss, nondecreasing 25/50/75/100 slice budgets, integrated pitched fragments across fixed seeds, and transport invalidation |
| FRACTURE | Presets, autonomous motion, digital/rate integration, exact bypass, silence, hostile extremes, finite/DC bounds | Run #60 passed all 30 macro presets, deterministic animated output, 1x–64x Rate participation, exact zero bypass, silence preservation, 192 kHz hostile NaN/Inf/extreme state, and full-chain stress |
| SMEAR | Exact bypass, pitched-grain history, deterministic state, intensity/high-frequency progression, hostile extremes | Run #60 passed cross-block cloud persistence, fixed-seed equality, exact zero bypass, finite hostile output, and deterministic 0/25/50/75/100 render checks |
...
| Full integration | Exact global order, variable blocks, discontinuity, hostile state/audio, bounded finite output | Run #60 passed 1/17/63/128/255/511/7/89-sample blocks through `SCRAMBLE -> FRACTURE -> SPECTRAL DRAW -> SMEAR -> OUTPUT`, including NaN/Inf injection and transport discontinuity |
