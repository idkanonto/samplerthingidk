...
|---|---|---|
| Build | Clean Release VST3 and Standalone builds | [Windows Release CI run #19](https://github.com/idkanonto/samplerthingidk/actions/runs/33880973821) at `e774868` built the Release VST3 and test targets, passed CTest, verified the bundle/module and license resources, and uploaded the inspected artifact; Standalone was not built in this run |
...
| State | Automation and project save/reopen, moved/missing sources | Run #19 tested source UUID, enabled state, Start, End, Source Key, Gain, Transpose, Fine Tune, Stretch, and Weight serialization plus parameter-only pool replacement; global APVTS state, DAW automation/save-reopen, and moved/missing files were not host-tested |
| Realtime | No I/O, blocking, allocation/reclamation, or stretch prep in callback | Immutable waveform/prepared data, snapshots, atomic global parameter reads, Note-On pitch resolution, background stretch publication, active-voice ownership, and deferred control-thread reclamation were code-inspected at `e774868`; applicable non-UI behavior passed run #19, but a profiler/stress test was not run |
