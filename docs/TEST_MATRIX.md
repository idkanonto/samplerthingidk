...
|---|---|---|
| Build | Clean Release VST3 and Standalone builds | [Windows Release CI run #32](https://github.com/idkanonto/samplerthingidk/actions/runs/33937562585) at `db18d0d` built the Release VST3 and test targets, passed CTest, verified the bundle/module and license resources, and uploaded the inspected artifact with SHA-256 `4df0fd69d8e194bf81bcb5c99b7cb861e00170b123a57b566536244a2265661b`; Standalone was not built in this run |
...
| State | Automation and project save/reopen, moved/missing sources | Run #29 compiled the appended `stepLength` APVTS ID and tested versioned Step Mask bit-state round-tripping plus the all-FX legacy fallback; source-state serialization has automated coverage, while global DAW automation/save-reopen and moved/missing files remain without a host test |
| Realtime | No I/O, blocking, allocation/reclamation, or stretch prep in callback | Fixed Take/Step/Chance arrays, audio-thread-owned capture/replay cursors, fixed UUID bytes, atomic UI requests/status, zero replay RNG, immutable prepared references, SampleManager retirement roots, and deferred control-thread reclamation were code-inspected at `db18d0d`; applicable behavior passed run #32, including explicit Take lifetime/eviction reclamation, but a profiler/stress test was not run |
